...
function moveElementDown(item: LayoutItem, yPos: number, layout: Layout) {
  ...
  for (let i = itemIndex+1; i < layout.length; i++) {
    ...
    console.log('item ' + item.i + ' y= ' + item.y + ' otherItem ' + otherItem.i + ' y='+ otherItem.y);
    // if we encounter something that is further down
    // we can break
    ...
    if (collides(item, otherItem)) {
      console.log('item ' + item.i + ' collides with ' + otherItem.i);
      otherItem.y = yPos + item.h;
    }
  }
  ...
  item.y = yPos;
  console.log('moved down ' + item.i + ' to ' + item.y);
}
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem)) {
      return moveElement(
        ...
        compactV ? fakeItem.y + 1 + fakeItem.h : undefined,
        isUserAction,
        preventCollision,
        compactType,
        cols
