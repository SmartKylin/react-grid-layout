...
function moveElementDown(item: LayoutItem, yPos: number, layout: Layout) {
  console.log('Compact: Move element down ' + item.i + ' to ' + yPos);
  ...
  for (let i = itemIndex+1; i < layout.length; i++) {
    ...
    if (collides(item, otherItem)) {
      moveElementDown(otherItem, yPos + item.h, layout);
    }
  }
...
export function compactItem(compareWith: Layout, l: LayoutItem, compactType: CompactType, cols: number, layout: Layout): LayoutItem {
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l)) {
      console.log('Compact: moving element up i=' + l.i + ' y= ' + l.y);
      l.y--;
    }
  } else if (compactH) {
    l.y = Math.min(bottom(compareWith), l.y);
  ...
  while((collides = getFirstCollision(compareWith, l))) {
    if (compactH) {
      ...
      moveElementDown(l, collides.y + collides.h, layout)
    }
...
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number,
  ...
  console.log('move element ' + l.i +  ' to y='+ y);
...
export function moveElementAwayFromCollision(layout: Layout, collidesWith: LayoutItem, itemToMove: LayoutItem,
  ...
  console.log('move away from collition: collidesWith=' + collidesWith.i + 'y=' + collidesWith.y + ' itemToMove= ' + itemToMove.i + ' y=' + itemToMove.y + ' h=' + itemToMove.h);
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem)) {
      return moveElement(
        ...
        compactV ? fakeItem.y + fakeItem.h + 1: undefined,
        isUserAction,
        preventCollision,
        compactType,
        cols
